              ...
              <div className="grid grid-cols-1 gap-3 lg:gap-4">
                ...
                <div className="grid grid-cols-3 gap-2 lg:hidden">
                  ...
                  <div>
                    ...
                    {(() => {
                      const modelEntry = getModelsForProvider(promptConfig.model).find(
                        (m) => m.id === (promptConfig as any).modelId,
                      );
                      if (!modelEntry?.reasoningCapable) return null;
                      const thinkingMode = (promptConfig as any).thinkingMode ?? "auto";
                      return (
                        <div className="mt-2 flex flex-wrap items-center gap-2">
                          <Select
                            value={thinkingMode}
                            onValueChange={(v) => updatePrompt(index, "thinkingMode" as any, v)}
                          >
                            <SelectTrigger className="h-7 text-xs w-32" data-testid={`select-thinking-mode-${index}`}>
                              <SelectValue placeholder="Reasoning" />
                            </SelectTrigger>
                            <SelectContent>
                              <SelectItem value="auto">Reasoning: Auto</SelectItem>
                              <SelectItem value="on">Reasoning: On</SelectItem>
                              <SelectItem value="off">Reasoning: Off</SelectItem>
                            </SelectContent>
                          </Select>
                          {thinkingMode === "on" && (
                            <Select
                              value={(promptConfig as any).thinkingEffort ?? "medium"}
                              onValueChange={(v) => updatePrompt(index, "thinkingEffort" as any, v)}
                            >
                              <SelectTrigger className="h-7 text-xs w-28" data-testid={`select-thinking-effort-${index}`}>
                                <SelectValue />
                              </SelectTrigger>
                              <SelectContent>
                      ...
                      return (
                        <div className="mt-2 flex flex-wrap items-center gap-2">
                          <Select
                            ...
                            onValueChange={(v) => updatePrompt(index, "thinkingMode" as any, v)}
                          >
                            <SelectTrigger className="h-7 text-xs w-32" data-testid={`select-thinking-mode-desktop-${index}`}>
                              <SelectValue placeholder="Reasoning" />
                            </SelectTrigger>
                            <SelectContent>
                              <SelectItem value="auto">Reasoning: Auto</SelectItem>
                          ...
                          {thinkingMode === "on" && (
                            <Select
                              ...
                              onValueChange={(v) => updatePrompt(index, "thinkingEffort" as any, v)}
                            >
                              <SelectTrigger className="h-7 text-xs w-28" data-testid={`select-thinking-effort-desktop-${index}`}>
                                <SelectValue />
                              </SelectTrigger>
                              <SelectContent>
                                <SelectItem value="low">Effort: Low</SelectItem>
