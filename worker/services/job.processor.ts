            ...
            if (dedupeEnabled && resultsMap.has(hmacKey)) {
              ...
              const callPromise = (async () => {
                ...
                const res = await llmService.processMessages({ systemText: processedSystem, userText: processedUser }, prompt.model, {
                  ...
                  // Pass per-prompt thinking controls through to the adapter (capability map enforces support).
                  thinkingMode: (prompt as any).thinkingMode,
                  thinkingEffort: (prompt as any).thinkingEffort,
                });
                return res;
              })();
