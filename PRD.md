...
- **Multi-LLM Integration**: Support for OpenAI GPT (GPT-5.5 line + 5.4 line + 5 + 4o/4.1), Anthropic Claude (Opus 4.7 / Sonnet 4.6 / Haiku 4.5), Google Gemini (3.1 Pro / 3 Flash / 3.1 Flash-Lite), Perplexity Sonar, and DeepSeek V4. Per-prompt reasoning / extended-thinking controls surface where the selected model supports them.
...
- **US18a (Thinking / Reasoning Controls):** As a user, when I pick a reasoning-capable model, I want a per-prompt `Reasoning: Auto / On / Off` selector (plus an effort level when On) so I can opt into extended thinking on slow, high-stakes prompts and opt out on cheap, simple ones. The control auto-hides for models that don't support it.
