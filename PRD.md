...
- **Multi-LLM Integration**: Support for OpenAI GPT, Perplexity Sonar, and Google Gemini models
...
- **US17 (API Key)**: As a user, I want to enter my API key for LLM service (Perplexity, ChatGPT/OpenAI, Gemini), so each request I make uses that API key and only the account associated with my API key is used for running queries
- **US18 (Model Selection):** As for user, I want to select the model (Perplexity, ChatGPT, Gemini) for each prompt that I add, so that only the model I've selected is used for running that prompt
...
3. **API Key Setup:** User navigates to the `Settings` page and enters API keys for OpenAI, Gemini, Perplexity, and/or DeepSeek. If the user begins at the dashboard without keys, a CTA banner directs them to Settings.
4. **Upload CSV:**
