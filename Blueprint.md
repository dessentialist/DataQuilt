...
- **Accessibility**: Keyboard navigability and ARIA roles applied; no formal WCAG audit performed (the prior `accessibility-validator.ts` module was a stub and has been removed — track real WCAG work in a dedicated initiative if/when it becomes a priority)
