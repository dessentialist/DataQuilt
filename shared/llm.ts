...
import { computeOpenAIEffectiveParams, computePerplexityEffectiveParams, computeGeminiEffectiveParams, computeDeepSeekEffectiveParams, getOpenAICapabilities } from "./llm.capabilities";
...
export const LLM_MODEL_CONFIGS = {
  ...
  gemini: {
    // Bumped to Gemini 3 Flash Preview per 2026-05 model refresh; 2.5 line is marked deprecated.
  ...
  deepseek: {
    // Legacy deepseek-chat alias is EOL 2026-07-24; default to V4 Flash.
    modelName: "deepseek-v4-flash",
    displayName: "DeepSeek V4 Flash",
  },
  anthropic: {
    // Bumped to Claude Sonnet 4.6 (the new balanced default) per 2026-05 model refresh.
...
export interface LLMServiceOptions {
   ...
   *   - "auto": use the provider/model default behavior (capability map decides).
   *   - "on": force thinking on if the model supports it; no-op otherwise (with a log).
   *   - "off": force thinking off if the model supports it; no-op otherwise.
   */
  thinkingMode?: "auto" | "on" | "off";
  /**
   * Effort hint for reasoning models that expose a level (OpenAI reasoning, Anthropic thinking budget tier).
   * Ignored on providers/models that don't expose a level knob.
   */
...
export class LLMService {
  ...
  async processMessages(
    ...
    try {
      const result = await this.withTimeoutAndRetries(async (signal) => {
        const msgs: BaseMessage[] = [new HumanMessage({ content: messagesInput.userText })];
  ...
  private getOrCreateModel(provider: SupportedModel, modelId: string, requested: LLMServiceOptions = {}): BaseChatModel {
    ...
    if (provider === "openai") {
      ...
      const eff = computeOpenAIEffectiveParams(modelId, requested);
      // Thinking/reasoning resolution: user override > capability default
      const resolvedEffort = this.resolveOpenAIReasoningEffort(eff.reasoningEffort, requested);
      console.log(`[OpenAIAdapter] params`, {
        provider,
        modelId,
        requested,
        effective: { temperature: eff.temperature, maxTokens: eff.maxTokens, reasoningEffort: resolvedEffort, apiSurface: eff.apiSurface },
      ...
      const model = new ChatOpenAI({
        ...
        // LangChain 1.x exposes reasoning controls for Responses-API models via `reasoning`.
        // We pass via modelKwargs to remain compatible across minor versions.
        ...(resolvedEffort
          ? { modelKwargs: { reasoning: { effort: resolvedEffort } } }
          : {}),
      } as any);
    ...
    if (provider === "deepseek") {
      ...
      // DeepSeek V4 toggles thinking via a request param, not the model ID.
      // The legacy `isReasoner = modelId === "deepseek-reasoner"` heuristic was tied to V3 and is obsolete.
      // Thinking mode: explicit user override beats the auto default.
      const wantsThinking =
        requested.thinkingMode === "on"
          ? true
          : requested.thinkingMode === "off"
            ? false
            : undefined; // auto — let capability map decide (currently defaults to false)
      const eff = computeDeepSeekEffectiveParams(modelId, {
        ...(requested as any),
        thinking: typeof wantsThinking === "boolean" ? wantsThinking : false,
      });
      console.log(`[DeepSeekAdapter] params`, {
        provider,
        modelId,
        requested,
        effective: { temperature: eff.temperature, maxTokens: eff.maxTokens, thinking: eff.thinking },
        sanitizedFields: eff.sanitizedFields,
      });
      const model = new ChatOpenAI({
        ...
        timeout: requested.timeoutMs ?? 10000,
        maxRetries: requested.maxRetries ?? 2,
        // V4 thinking toggle: passed as an OpenAI-compatible extra body field.
        modelKwargs: eff.thinking ? { thinking: true } : undefined,
      } as any);
      this.models.set(cacheKey, model);
      return model;
    }
    ...
    if (provider === "anthropic") {
      ...
      // Anthropic extended thinking is supported on Claude 4.x. We pass `thinking` via modelKwargs
      // when the user explicitly enabled it; "auto" lets the model decide.
      const thinkingBlock = this.resolveAnthropicThinkingBlock(modelId, requested);
      console.log(`[AnthropicAdapter] params`, {
        provider,
        modelId,
        temperature,
        maxTokens: requested.maxTokens ?? 1024,
        thinking: thinkingBlock ?? "auto",
      });
      // LangChain @langchain/anthropic 1.x removed `timeout` from constructor.
      // Request-level cancellation is handled by withTimeoutAndRetries via AbortSignal.
      const model = new ChatAnthropic({
        ...
        maxTokens: requested.maxTokens ?? 1024,
        maxRetries: requested.maxRetries ?? 2,
        ...(thinkingBlock ? { modelKwargs: { thinking: thinkingBlock } } : {}),
      } as any);
      this.models.set(cacheKey, model);
      return model;
    }
  ...
  async processPrompt(
    ...
    try {
      ...
      const result = await this.withTimeoutAndRetries(async (signal) => {
        const messages: BaseMessage[] = [new HumanMessage({ content: prompt })];
        if (model === "gemini") {
  ...
  /**
   * Resolve the effective OpenAI reasoning effort from the user's thinking config.
   * - thinkingMode "off": drop reasoning entirely.
   * - thinkingMode "on": use thinkingEffort if provided, else fall back to the capability default.
   * - thinkingMode "auto" or unset: use capability default.
   * Returns undefined for non-reasoning models (when defaultEffort is undefined).
   */
  private resolveOpenAIReasoningEffort(
    defaultEffort: "low" | "medium" | "high" | undefined,
    requested: LLMServiceOptions,
  ): "low" | "medium" | "high" | undefined {
    if (!defaultEffort) return undefined; // non-reasoning model
    if (requested.thinkingMode === "off") return undefined;
    if (requested.thinkingMode === "on") return requested.thinkingEffort ?? defaultEffort;
    return requested.thinkingEffort ?? defaultEffort;
  }

  /**
   * Build the Anthropic extended-thinking block, or return null when thinking should not be applied.
   * Claude 4.x supports extended thinking via `thinking: { type: "enabled", budget_tokens: N }`.
   * On older Claude models this is silently ignored by the API; we log and skip for clarity.
   */
  private resolveAnthropicThinkingBlock(
    modelId: string,
    requested: LLMServiceOptions,
  ): { type: "enabled"; budget_tokens: number } | null {
    const isClaude4 = /^claude-(opus|sonnet|haiku)-4-/.test(modelId);
    if (!isClaude4) return null;
    if (requested.thinkingMode !== "on") return null;
    const effort = requested.thinkingEffort ?? "medium";
    // Map effort levels to budget tokens. Conservative defaults; users can request more via thinkingEffort.
