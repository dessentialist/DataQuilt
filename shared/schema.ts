...
export const promptTemplates = pgTable("prompt_templates", {
  ...
  // Extended thinking / reasoning toggle. App-level Zod enforces the enum.
  thinkingMode: text("thinking_mode", { enum: ["auto", "on", "off"] }).notNull().default("auto"),
  thinkingEffort: text("thinking_effort", { enum: ["low", "medium", "high"] }),
  createdAt: timestamp("created_at", { withTimezone: true }).defaultNow().notNull(),
});
...
// Thinking / reasoning controls — surfaced per-prompt.
// `auto` = let the capability map / provider default decide; `on`/`off` are explicit user choices.
// `thinkingEffort` is only meaningful when thinking is effectively on; providers without effort levels ignore it.
export const thinkingModeSchema = z.enum(["auto", "on", "off"]);
export const thinkingEffortSchema = z.enum(["low", "medium", "high"]);
...
export const promptConfigSchema = z.object({
  ...
  // Optional in PromptConfig: undefined === "auto" (let provider/capability defaults decide).
