 ...
 * Keep names stable for idempotency checks — the seeding service uses names as the de-dupe key.
 *
 * DEFAULT_PROMPT_TEMPLATES is intentionally empty: prompt-template content is highly task-specific,
 * so users build their own from the in-app library and save them. Add to this list only after the
 * content has been validated end-to-end (preview run on a representative dataset).
 *
 * DEFAULT_SYSTEM_TEMPLATES below contains the curated starter system messages.
 */
