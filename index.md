...
- **OpenAI**: `ChatOpenAI` from `@langchain/openai` v1.x (GPT-5.5 line, GPT-5.4 line, GPT-5, GPT-4o, GPT-4.1).
- **Anthropic**: `ChatAnthropic` from `@langchain/anthropic` v1.x (Claude Opus 4.7, Sonnet 4.6, Haiku 4.5; older 4.5 + 3.5 IDs retained as deprecated).
- **Gemini**: `ChatGoogleGenerativeAI` from `@langchain/google-genai` v2.x (Gemini 3.1 Pro / 3 Flash / 3.1 Flash-Lite preview; 2.x family retained as deprecated).
- **Perplexity**: `ChatPerplexity` from `@langchain/community/chat_models/perplexity` v1.x (`sonar`, `sonar-pro`, `sonar-reasoning-pro`).
- **DeepSeek**: OpenAI-compatible via `ChatOpenAI` with `baseURL: https://api.deepseek.com` — `deepseek-v4-flash` and `deepseek-v4-pro` (1M context; dual thinking/non-thinking via request param). Legacy `deepseek-chat`/`deepseek-reasoner` aliases were dropped (EOL 2026-07-24). See [DeepSeek API Docs](http://api-docs.deepseek.com/).

#### Curated Model Allowlist (verified 2026-05-17 — see `shared/llm.models.ts`)

| Provider    | Model ID                          | Display name                       | Reasoning-capable | Status      |
|-------------|-----------------------------------|------------------------------------|-------------------|-------------|
| OpenAI      | gpt-5.5                            | GPT-5.5                            | Yes               | Active      |
| OpenAI      | gpt-5.5-pro                        | GPT-5.5 Pro (Responses API only)   | Yes               | Active      |
| OpenAI      | gpt-5.4-mini                       | GPT-5.4-mini                       | Yes               | Active      |
| OpenAI      | gpt-5.4-nano                       | GPT-5.4-nano                       | No                | Active      |
| OpenAI      | gpt-5                              | GPT-5                              | Yes               | Active      |
| OpenAI      | gpt-5-mini                         | GPT-5-mini                         | Yes               | Active      |
| OpenAI      | gpt-4o                             | GPT-4o                             | No                | Active      |
| OpenAI      | gpt-4o-mini                        | GPT-4o-mini                        | No                | Active      |
| OpenAI      | gpt-4.1                            | GPT-4.1                            | No                | Active      |
| OpenAI      | gpt-4.1-mini                       | GPT-4.1-mini                       | No                | Active      |
| Anthropic   | claude-opus-4-7                    | Claude Opus 4.7                    | Yes               | Active      |
| Anthropic   | claude-sonnet-4-6                  | Claude Sonnet 4.6                  | Yes               | Active      |
| Anthropic   | claude-haiku-4-5-20251001          | Claude Haiku 4.5                   | Yes               | Active      |
| Anthropic   | claude-sonnet-4-5-20250929         | Claude Sonnet 4.5                  | Yes               | Deprecated  |
| Anthropic   | claude-3-5-sonnet-latest           | Claude 3.5 Sonnet                  | No                | Deprecated  |
| Anthropic   | claude-3-5-haiku-latest            | Claude 3.5 Haiku                   | No                | Deprecated  |
| Gemini      | gemini-3.1-pro-preview             | Gemini 3.1 Pro (Preview)           | Yes               | Active      |
| Gemini      | gemini-3-flash-preview             | Gemini 3 Flash (Preview)           | Yes               | Active      |
| Gemini      | gemini-3.1-flash-lite-preview      | Gemini 3.1 Flash-Lite (Preview)    | No                | Active      |
| Gemini      | gemini-2.5-pro                     | Gemini 2.5 Pro                     | Yes               | Deprecated  |
| Gemini      | gemini-2.5-flash                   | Gemini 2.5 Flash                   | Yes               | Deprecated  |
| Gemini      | gemini-2.5-flash-lite              | Gemini 2.5 Flash-Lite              | No                | Deprecated  |
| Gemini      | gemini-2.0-flash                   | Gemini 2.0 Flash                   | No                | Deprecated  |
| Perplexity  | sonar                              | Sonar                              | No                | Active      |
| Perplexity  | sonar-pro                          | Sonar Pro                          | No                | Active      |
| Perplexity  | sonar-reasoning-pro                | Sonar Reasoning Pro                | Yes               | Active      |
| DeepSeek    | deepseek-v4-flash                  | DeepSeek V4 Flash                  | Yes               | Active      |
| DeepSeek    | deepseek-v4-pro                    | DeepSeek V4 Pro                    | Yes               | Active      |
...
- All Perplexity models (`sonar`, `sonar-pro`, `sonar-reasoning-pro`) use deterministic enrichment defaults: temperature=0, safe token caps, and capability-aware sanitization.
...
- Gemini models (3.x preview line and the deprecated 2.x family) use deterministic enrichment defaults: temperature=0 and safe `maxOutputTokens`.
- Logs include requested vs effective parameters and sanitized fields; cache keys reflect temperature buckets.
...
- Reasoning-capable models (`gpt-5.5`, `gpt-5.5-pro`, `gpt-5.4-mini`, `gpt-5`, `gpt-5-mini`) are invoked via a Responses-style configuration path. Classic sampling knobs (temperature/top_p/penalties) are omitted; `reasoning.effort` defaults to `high` (5.5/5) or `medium` (5.4-mini) and may be overridden per-prompt via `thinkingEffort`.
- Non-reasoning models (`gpt-5.4-nano`, `gpt-4o`, `gpt-4o-mini`, `gpt-4.1`, `gpt-4.1-mini`) use Chat semantics with deterministic defaults for enrichment (temperature=0, top_p=1, penalties=0) and conservative `max_tokens`.
- A per-prompt `thinkingMode` (`auto` / `on` / `off`) flows from `promptsConfig` through `LLMService.processMessages` into the provider-specific payload: OpenAI `reasoning.effort`, Anthropic `thinking: { type, budget_tokens }`, DeepSeek `thinking: true`.
