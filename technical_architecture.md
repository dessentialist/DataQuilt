...
### **AI & LLM Integration** (lineup verified 2026-05-17)
- **LangChain**: Core framework for LLM integration and orchestration. All `@langchain/*` packages on v1.x (anthropic 1.3.x, openai 1.4.x, google-genai 2.1.x, core 1.1.x, community 1.1.x). Mixed-message arrays are typed `BaseMessage[]` in 1.x.
- **OpenAI API**: `ChatOpenAI` from `@langchain/openai`. GPT-5.5 / 5.5-pro (Responses API) / 5.4-mini / 5.4-nano / 5 / 5-mini / 4o(-mini) / 4.1(-mini).
- **Anthropic API**: `ChatAnthropic` from `@langchain/anthropic`. Active: `claude-opus-4-7`, `claude-sonnet-4-6`, `claude-haiku-4-5-20251001`. Claude 4.5 + 3.5 kept as `deprecated: true`; Claude 3 base models removed. Extended thinking on Claude 4.x is plumbed via `modelKwargs.thinking`.
- **Google Gemini API**: `ChatGoogleGenerativeAI` from `@langchain/google-genai`. Active: Gemini 3.1 Pro Preview, Gemini 3 Flash Preview, Gemini 3.1 Flash-Lite Preview. Gemini 2.x retained as `deprecated: true`.
- **Perplexity API**: `ChatPerplexity` from `@langchain/community`. Models: `sonar`, `sonar-pro`, `sonar-reasoning-pro` (non-pro `sonar-reasoning` retired by Perplexity).
- **DeepSeek API**: OpenAI-compatible via LangChain `ChatOpenAI` using `baseURL: https://api.deepseek.com`. Models: `deepseek-v4-flash`, `deepseek-v4-pro` (1M context, dual thinking/non-thinking modes via request param). Legacy `deepseek-chat` / `deepseek-reasoner` aliases were dropped from the registry (they end-of-life on 2026-07-24).
- **Per-prompt thinking / reasoning controls**: `promptConfigSchema` carries optional `thinkingMode` ∈ {`auto`,`on`,`off`} and `thinkingEffort` ∈ {`low`,`medium`,`high`}. `LLMService` translates these into the provider's wire format (OpenAI `reasoning.effort`, Anthropic `thinking: { type, budget_tokens }`, DeepSeek `thinking: true`).
...
- Reasoning models (`gpt-5.5`, `gpt-5.5-pro`, `gpt-5.4-mini`, `gpt-5`, `gpt-5-mini`) are handled via a Responses-style path: classic sampling knobs are omitted, `reasoning.effort` is used (default: `high` for 5.5/5; `medium` for 5.4-mini), and safe token limits are applied. `gpt-5.5-pro` is Responses-API-only.
- Non-reasoning models (`gpt-5.4-nano`, `gpt-4o`, `gpt-4o-mini`, `gpt-4.1`, `gpt-4.1-mini`) use Chat defaults optimized for deterministic enrichment: temperature=0, top_p=1, penalties=0.
...
- Per-prompt `thinkingMode`/`thinkingEffort` overrides the capability default — `off` drops the `reasoning.effort` field entirely; `on` uses the supplied effort (or capability default).
...
- **Provider Management**: LangChain v1.x packages per LLM service (`@langchain/openai`, `@langchain/anthropic`, `@langchain/google-genai`, `@langchain/community`, `@langchain/core`).
- **Model Registry** (canonical source: `shared/llm.models.ts`, last refreshed 2026-05-17):
  - **OpenAI**: `gpt-5.5`, `gpt-5.5-pro`, `gpt-5.4-mini`, `gpt-5.4-nano`, `gpt-5`, `gpt-5-mini`, `gpt-4o`, `gpt-4o-mini`, `gpt-4.1`, `gpt-4.1-mini`
  - **Anthropic**: `claude-opus-4-7`, `claude-sonnet-4-6`, `claude-haiku-4-5-20251001` (active); `claude-sonnet-4-5-20250929`, `claude-3-5-sonnet-latest`, `claude-3-5-haiku-latest` (deprecated)
  - **Gemini**: `gemini-3.1-pro-preview`, `gemini-3-flash-preview`, `gemini-3.1-flash-lite-preview` (active); `gemini-2.5-pro`, `gemini-2.5-flash`, `gemini-2.5-flash-lite`, `gemini-2.0-flash` (deprecated)
  - **DeepSeek**: `deepseek-v4-flash`, `deepseek-v4-pro` (active; both support thinking via request param)
  - **Perplexity**: `sonar`, `sonar-pro`, `sonar-reasoning-pro`
- Each `ModelEntry` carries a `deprecated?: boolean` flag so the UI can group legacy entries under "Legacy" rather than removing them outright.
- **Explicit ModelId**: Required per prompt; no provider defaults at the API; validated at the application layer via Zod (`isAllowedModelId`).
...
- **Per-prompt thinking config**: `thinkingMode` ∈ `auto`|`on`|`off` and optional `thinkingEffort` are forwarded from `promptsConfig` through the worker (`worker/services/job.processor.ts`) and the preview path (`server/services/jobs.service.ts`) into `LLMService.processMessages`. Helpers in `shared/llm.ts` translate these to provider-specific payloads.
...
- **Model Configuration**: Provider-specific model selection and parameters via `shared/llm.capabilities.ts` (OpenAI, Gemini, Perplexity, DeepSeek maps).
