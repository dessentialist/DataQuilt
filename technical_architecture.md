...
```
CSV Row → Variable Substitution → Provider Selection (OpenAI/Gemini/Perplexity) → Model Selection (allowlisted modelId) → Per-Prompt Dedupe Check (HMAC key) → LangChain LLM Call (if miss/inflight) → Response Processing → Output Generation → Progress Update
...
- **Provider Abstraction**: Unified interface for OpenAI, Gemini, and Perplexity
...
- **Model Registry**: Curated allowlist per provider (OpenAI: gpt-4o, gpt-4o-mini, gpt-4.1, gpt-4.1-mini; Gemini: gemini-2.5-pro/flash/flash-lite; Perplexity: sonar/sonar-pro/sonar-reasoning/sonar-reasoning-pro; DeepSeek: deepseek-chat, deepseek-reasoner)
